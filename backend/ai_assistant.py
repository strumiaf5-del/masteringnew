...
- Podés recomendar valores concretos de LUFS, ceiling de limiter, ratios de compresión, EQ, etc.
...
def chat(user_message: str, history: Optional[list] = None,
         ...
         platform: Optional[str] = None) -> str:
    """Envía un mensaje al asistente de IA y devuelve la respuesta en texto plano.
    ...
    if not text:
        try:
            ...
            logger.warning(f"Respuesta de chat vacía de Gemini (finish_reason={finish_reason}).")
        except Exception:
            pass
    return (text or "").strip() or "No obtuve respuesta del modelo. Probá reformular la pregunta."
...
- True peak / clipping: si true_peak_db o clipping_ratio ya muestran problemas, usá limiter_ceiling \
más conservador (≤0.94) y no agregues makeup gain innecesario.
...
def decide_mastering(analysis: Optional[dict], platform_options: list,
    ...
    field_defs = {k: (float, _NEUTRAL_PARAMS.get(k, 0.0)) for k in PARAM_RANGES}
    ...
    ranges_block = "\n".join(f"- {k}: [{lo}, {hi}]" for k, (lo, hi) in PARAM_RANGES.items())
    ...
    for key, (lo, hi) in PARAM_RANGES.items():
        clamped = _clamp(data.get(key, _NEUTRAL_PARAMS.get(key)), lo, hi)
